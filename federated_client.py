        ...
        with torch.no_grad():
            ...
            for step, batch_index in enumerate(np.random.permutation(range(0, steps)), start=1):
                ...
                for index, (model_sensor, output_mask) in enumerate(zip(self.model_sensors, self.output_mask), start=1):
                    ...
                    # Output
                    df_out = self.df_test[self.test_output_indices[batch_index * constants.daics.BATCH_SIZE: batch_index * constants.daics.BATCH_SIZE + constants.daics.BATCH_SIZE].flatten()][:, output_mask]
